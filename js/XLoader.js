...
THREE.XLoader.prototype = {
  ...
  readMeshSection: function (_baseOffset) {
    var scope = this;
    if (!scope.nowFrameName || scope.nowFrameName == "") {
      scope.beginFrame("");
    }
  ...
  readMaterial: function (_dataLine) {
    ...
    var scope = this;
    scope.nowMat = new THREE.MeshPhongMaterial({
  ...
  SectionRead: function () {
    ...
    if (find3 > find2 && find2 > -1) {
      ...
      } else if (line.indexOf("MeshNormals ") > -1) {
        // skip
      ...
      section = scope.getNextSection(scope.readedLength, find, find3);
      scope.readedLength = find3 + 1;
  ...
  beginFrame: function (line) {
    ...
    if (findindex > -1) {
      ...
      scope.nowFrameName = "Frame" + scope.loadingXdata.FrameInfo_Raw.length;
    }
  ...
  readFinalize: function () {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      } else if (scope.loadingXdata.FrameInfo_Raw[keys[i]].Geometry != null) {
        scope.MakeOutputGeometry(keys[i]);
        scope.loadingXdata.FrameInfo.push(scope.loadingXdata.FrameInfo_Raw[keys[i]].Mesh);
      }
    ...
    if (scope.loadingXdata.FrameInfo_Raw[nowFrameName].Geometry != null) {
      ...
      if (scope.loadingXdata.FrameInfo_Raw[nowFrameName].BoneInfs != null && scope.loadingXdata.FrameInfo_Raw[nowFrameName].BoneInfs.length) {
        ...
        for (var m = 0; m < keys.length; m++) {
          ...
          b.name = keys[m];
          BoneDics_Name[b.name] = putBones.length;
          b.baseMatrix = scope.loadingXdata.FrameInfo_Raw[keys[m]].FrameTransformMatrix;
          // b.applyMatrix(scope.loadingXdata.FrameInfo_Raw[keys[m]].FrameTransformMatrix);
          putBones.push(b);
        }
        ...
        for (var _m = putBones.length - 1; _m > 0; _m--) {

          if (putBones[_m].parent != null) {
            var tmpM = new THREE.Matrix4();
            var tmpM2 = new THREE.Matrix4();
            tmpM2.getInverse(putBones[_m].parent.baseMatrix);
            tmpM.copy(putBones[_m].baseMatrix);
            var resM = new THREE.Matrix4();
            resM.multiplyMatrices(tmpM2, tmpM);
            putBones[_m].applyMatrix(resM);
            putBones[_m].updateMatrix();
          }
